...
contract("KudosWorkerRewardSystemTests", function([deployerAddress, userAddress, workerAddress, businessAddress]) {
  ...
  it("deployer should be able to register growth pool reward system", async function() {
    ...
    await kudos.registerGrowthPoolRewardSystem(numberOfRewardSteps, numberOfRewardCycles, numberOfRewardLevels, levelRewards, ipfsHash);
    ...
    Number(growthPoolRewardSystemStruct[1]).should.equal(numberOfRewardCycles);
  ...
  it("non-deployer should not be able to register growth pool reward system", async function() {
    ...
    await kudos.registerGrowthPoolRewardSystem(numberOfRewardSteps, numberOfRewardCycles, numberOfRewardLevels, levelRewards, ipfsHash, {from: businessAddress}).should.be.rejected;
  });
});
